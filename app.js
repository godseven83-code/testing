...
function init() {
  ...
  setTimeout(() => {
    ...
    if (mobileMenuBtn) {
      console.log('Mobile menu button visible:', window.getComputedStyle(mobileMenuBtn).display !== 'none');
      console.log('Mobile menu button dimensions:', mobileMenuBtn.offsetWidth, 'x', mobileMenuBtn.offsetHeight);
    }
    ...
    if (window.innerWidth <= 768) {
      ...
      if (mobileMenuBtn && mobileNav) {
        ...
        mobileMenuBtn.click();
        setTimeout(() => {
          console.log('Mobile nav active after click:', mobileNav.classList.contains('active'));
...
function setupMobileBrowserFixes() {
  ...
  function improveTouchHandling() {
    ...
    buttons.forEach(button => {
      // Skip mobile menu button - it has its own dedicated handlers
  ...
  // SIMPLE MOBILE NAVIGATION - NO COMPLEX MENU NEEDED
  function setupSimpleMobileNav() {
    console.log('Setting up simple mobile navigation...');
    
    // Find all navigation links (both desktop and mobile)
    const allNavLinks = document.querySelectorAll('.simple-nav-link[data-page], .nav-link[data-page]');
    ...
    console.log('Simple mobile navigation setup complete - found', allNavLinks.length, 'links');
  }
  
  // Setup simple navigation immediately
  setupSimpleMobileNav();
  
  // Additional mobile touch event setup with aggressive mobile detection
  if (isMobile || window.innerWidth <= 768) {
    console.log('Setting up additional mobile touch handlers');
    
    // Setup checkout button specifically
    const checkoutBtn = document.querySelector('#checkout-btn');
    if (checkoutBtn) {
      console.log('Found checkout button, removing any duplicate handlers');
      
      // Remove any existing direct handlers to prevent duplicates
      checkoutBtn.onclick = null;
      checkoutBtn.ontouchend = null;
      console.log('Removed duplicate checkout handlers - using unified handlers instead');
    }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // SUPER SIMPLE MOBILE MENU - Just the basics
  setTimeout(() => {
    console.log('=== SETTING UP SIMPLE MOBILE MENU ===');
    ...
    if (btn && nav) {
      ...
      nav.querySelectorAll('.nav-link').forEach(link => {
        link.onclick = function(e) {
          ...
          showPage(page); // Navigate
        };
      });
      
      console.log('✅ Simple mobile menu setup complete');
    } else {
      console.error('Mobile menu elements not found');
    }
  }, 1000);
  
  // Simplified mobile-friendly event delegation
  const isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
  ...
  function handleUniversalClick(e) {
    ...
    console.log('Universal click on:', target, 'Type:', e.type);
    ...
    if (target.classList.contains('view-product-btn') || 
      ...
      console.log('Product action detected');
    ...
    if (target.classList.contains('quantity-btn')) {
      console.log('Cart action detected');
      handleCartActions(e);
      return;
    }
    ...
    if (target.closest('.category-card')) {
      console.log('Category filter detected');
  ...
  // Add both click and touchend for maximum compatibility
  document.addEventListener('click', handleUniversalClick);
  
  if (isMobile) {
    // For mobile, also listen to touchend but with a slight delay
  ...
  // Skip complex mobile menu setup - using simple approach instead
  console.log('Skipping complex mobile menu - using simplified version');
  ...
  if (checkoutBtn) {
    ...
    function triggerCheckout(e) {
      ...
      if (window.innerWidth <= 768) {
        console.log('Applying mobile checkout styles');
        // Prevent body scroll
    ...
    // Unified event handling for checkout to prevent duplicates
    let isCheckoutProcessing = false;
    
    function handleCheckout(e) {
      if (isCheckoutProcessing) {
        console.log('Checkout already processing, ignoring');
        return;
      }
    ...
    if (isMobile || window.innerWidth <= 768) {
      checkoutBtn.addEventListener('touchstart', function(e) {
        console.log('Checkout button touchstart');
        this.style.transform = 'scale(0.95)';
        this.style.opacity = '0.8';
      });
      
      checkoutBtn.addEventListener('touchend', function(e) {
        console.log('Checkout button touchend');
        // Handle the checkout action with delay to prevent conflicts
        setTimeout(() => {
          if (!isCheckoutProcessing) {
            handleCheckout(e);
          }
