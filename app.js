...
function setupMobileBrowserFixes() {
  ...
  if (isMobile || window.innerWidth <= 768) {
    ...
    // Setup mobile menu button specifically
    const mobileMenuBtn = document.querySelector('#mobile-menu-btn');
    if (mobileMenuBtn) {
      console.log('Found mobile menu button, removing any duplicate handlers');
      
      // Remove any existing direct handlers to prevent duplicates
      mobileMenuBtn.onclick = null;
      mobileMenuBtn.ontouchend = null;
      console.log('Removed duplicate mobile menu handlers - using unified handlers instead');
    }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function handleUniversalClick(e) {
    ...
    // Skip mobile menu button - it has its own dedicated handler
    if (target.id === 'mobile-menu-btn' || target.closest('#mobile-menu-btn')) {
      console.log('Skipping mobile menu - has dedicated handler');
      return;
    }
  ...
  if (mobileMenuBtn && mobileNav) {
    ...
    let isToggling = false; // Prevent rapid multiple toggles
    ...
    // Add ONLY the click event listener (works on both mobile and desktop)
    mobileMenuBtn.addEventListener('click', toggleMobileMenu);
    
    // Visual feedback only - no action handlers
    mobileMenuBtn.addEventListener('touchstart', function(e) {
      console.log('Mobile menu touchstart - visual only');
      this.style.transform = 'scale(0.95)';
      this.style.backgroundColor = 'var(--color-primary)';
    }, { passive: true });
    
    mobileMenuBtn.addEventListener('touchend', function(e) {
      console.log('Mobile menu touchend - visual only');
